...
export default (introspectionResults) => (aorFetchType, resource, queryType) => (res) => {
  const response = res.data;
  const tables = introspectionResults.tables;
  if (tables[queryType.name].isTarget) {
    response.items = addFakeIds(response.items, tables[queryType.name].keys);
  }
